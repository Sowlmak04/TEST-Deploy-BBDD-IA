// ---------- Modal detalle (pendientes + vistas) ----------
    ...
    function startEditFromDetail() {
      ...
      const ownedPhysicalField = form.elements.namedItem("ownedPhysical");
      if (ownedPhysicalField) ownedPhysicalField.checked = Boolean(item.ownedPhysical);
      ...
      if (isVistas) {
        ...
        const raterStates = typeof LibraryModel !== "undefined" && LibraryModel.getRaters
          ? LibraryModel.getRaters(item)
          : { adri: { watched: item.ratingAdri !== "" && item.ratingAdri != null }, laura: { watched: item.ratingLaura !== "" && item.ratingLaura != null } };
        ["adri","laura"].forEach(id => {
          const unseen=form.querySelector(`[data-rater-unseen="${id}"]`);
          if(unseen) unseen.checked=raterStates[id]?.watched===false;
          const notes=form.querySelector(`textarea[name="notes${id==="adri"?"Adri":"Laura"}"]`);
          if(notes) notes.disabled=raterStates[id]?.watched===false;
        });
// Set estrellas usando el widget
        form.querySelectorAll('[data-widget="rating"]').forEach(w => {
          ...
          const rid=target.includes("rating-adri")?"adri":target.includes("rating-laura")?"laura":"";
          if(rid){const disabled=raterStates[rid]?.watched===false;w.classList.toggle("raterDisabled",disabled);w.setAttribute("aria-disabled",disabled?"true":"false");}
        });
      }
    ...
    function renderDetailAsForm(item, { isSeries, isVistas }) {
      ...
      if (isVistas) {
        const detailRaters = typeof LibraryModel !== "undefined" && LibraryModel.getRaters ? LibraryModel.getRaters(item) : null;
        const detailRaterValue = (id, legacyValue) => {
          const state = detailRaters?.[id];
          if (state?.watched === false) return "No la ha visto";
          if (state && (state.rating === "" || state.rating == null)) return "Sin valorar";
          return state?.rating ?? legacyValue ?? "";
        };
        rows.push(field(`Valoración ${getRaterDisplayName("adri")}`, detailRaterValue("adri", item.ratingAdri)));
        rows.push(field(`Valoración ${getRaterDisplayName("laura")}`, detailRaterValue("laura", item.ratingLaura)));
        ...
        rows.push(`
          <div class="twoNotes">
            <div class="field">
              <label>Notas ${escapeHtml(getRaterDisplayName("adri"))}</label>
              <textarea readonly>${escapeHtml(item.notesAdri ?? "")}</textarea>
            </div>
            <div class="field">
              <label>Notas ${escapeHtml(getRaterDisplayName("laura"))}</label>
      ...
      if (item.ownedPhysical) rows.push(field("Colección propia", "Sí · copia física"));
      if (item.priority) {
      ...
      if (isSeries && item.tmdbId) {
        ...
        if (item.tmdbStatus) {
          rows.push(field("Estado en TMDb", productionStatusLabel(item.tmdbStatus)));
        }
      ...
      if (isSeries && isVistas) {
        const mainSeasons = Array.isArray(item.seasonsData) && item.seasonsData.length
          ? item.seasonsData
          : Array.from(
              { length: Math.max(0, Number(item.seasons) || 0) },
              (_, index) => ({
        ...
        if (mainSeasons.length) {
          const seasonRatings = item.seasonRatings && typeof item.seasonRatings === "object"
            ? item.seasonRatings
            : {};

          rows.push(`
            <div class="seasonRatingsPanel" style="grid-column:1/-1;">
              <button
                class="seasonRatingsDisclosure"
                type="button"
                data-season-ratings-panel-toggle
                aria-expanded="false"
              >
                <span class="seasonRatingsDisclosureIcon" aria-hidden="true">▸</span>
                <span>
                  <strong class="seasonRatingsTitle">Valoración por temporadas</strong>
                  <small>Tus valoraciones personales por temporada.</small>
                </span>
              </button>

              <div class="seasonRatingsPanelBody" data-season-ratings-panel-body hidden>
                <div class="seasonRatingsHeader">
                  <span>Notas personales por temporada</span>
                  <button class="toolBtn" type="button" data-season-ratings-toggle>
                    Ver valoraciones
                  </button>
                </div>

                <div class="seasonRatingsEditor" data-season-ratings-editor hidden>
                ${mainSeasons.map(season => {
                  const number = Number(season.seasonNumber) || 0;
                  const ratings = seasonRatings[String(number)] || {};
                  const adri = ratings.adri === "" || ratings.adri == null ? "" : ratings.adri;
                  const laura = ratings.laura === "" || ratings.laura == null ? "" : ratings.laura;
                  const label = season.name && season.name !== `Temporada ${number}`
                    ? `T${number} · ${escapeHtml(season.name)}`
                    : `Temporada ${number}`;

                  return `
                    <div class="seasonRatingRow" data-season-number="${number}">
                      <strong>${label}</strong>
                      <label>
                        <span>${escapeHtml(getRaterDisplayName("adri"))}</span>
                        <input
                          type="text"
                          inputmode="decimal"
                          autocomplete="off"
                          data-season-rating-user="adri"
                          value="${escapeHtml(adri)}"
                          placeholder="—"
                          aria-label="Valoración ${escapeHtml(getRaterDisplayName("adri"))} temporada ${number}"
                        >
                      </label>
                      <label>
                        <span>${escapeHtml(getRaterDisplayName("laura"))}</span>
                        <input
                          type="text"
                          inputmode="decimal"
                          autocomplete="off"
                          data-season-rating-user="laura"
                          value="${escapeHtml(laura)}"
                          placeholder="—"
                          aria-label="Valoración ${escapeHtml(getRaterDisplayName("laura"))} temporada ${number}"
                        >
                      </label>
                    </div>
                  `;
                ...
                <div class="seasonRatingsActions">
                  <span class="seasonRatingsHint">0–10 · máximo 2 decimales · vacío = sin valorar</span>
                  <button class="btnPrimary" type="button" data-season-ratings-save>
                    Guardar valoraciones
                  </button>
                </div>
                  <div class="seasonRatingsMessage" data-season-ratings-message aria-live="polite"></div>
                </div>
              </div>
            </div>
          `);
    ...
    function parseSeasonRating(raw) {
    ...
    detailBody.addEventListener("click", event => {
      const panelToggle = event.target.closest("[data-season-ratings-panel-toggle]");
      if (panelToggle) {
        const panel = panelToggle.closest(".seasonRatingsPanel");
        const body = panel?.querySelector("[data-season-ratings-panel-body]");
        const icon = panelToggle.querySelector(".seasonRatingsDisclosureIcon");
        if (!body) return;
      ...
      const toggle = event.target.closest("[data-season-ratings-toggle]");
      if (toggle) {
        const editor = detailBody.querySelector("[data-season-ratings-editor]");
        if (!editor) return;
        editor.hidden = !editor.hidden;
        toggle.textContent = editor.hidden ? "Ver valoraciones" : "Ocultar valoraciones";
        return;
      }

      const save = event.target.closest("[data-season-ratings-save]");
      if (!save) return;
      if (currentDetail.kind !== "series" || currentDetail.status !== "vistas") return;

      const rows = [...detailBody.querySelectorAll("[data-season-number]")];
      const nextRatings = {};
      ...
      rows.forEach(row => {
        ...
        const adriInput = row.querySelector('[data-season-rating-user="adri"]');
        const lauraInput = row.querySelector('[data-season-rating-user="laura"]');
        const adri = parseSeasonRating(adriInput?.value);
        const laura = parseSeasonRating(lauraInput?.value);

        if (adri === null) invalidInput = adriInput;
        if (laura === null && !invalidInput) invalidInput = lauraInput;
        if (invalidInput) return;

        if (adri !== "" || laura !== "") {
          nextRatings[String(seasonNumber)] = { adri, laura };
        }
      });

      const message = detailBody.querySelector("[data-season-ratings-message]");
      if (invalidInput) {
        invalidInput.focus();
        if (message) {
          message.textContent = "Revisa las notas: deben estar entre 0 y 10 y tener como máximo dos decimales.";
          message.className = "seasonRatingsMessage error";
        }
        return;
      }

      LibraryService.update(currentDetail.listKey, currentDetail.id, current => ({
        ...current,
        seasonRatings: nextRatings,
        updatedAt: Date.now()
      }));

      if (message) {
        message.textContent = "Valoraciones por temporada guardadas ✓";
        message.className = "seasonRatingsMessage ok";
      }
      if (typeof showToast === "function") {
        showToast("Valoraciones por temporada guardadas ✓");
      }
      ...
      if (log.length === 0) {
        ...
        historyBody.innerHTML = `
          <div class="historyList">
            ${ordered.map((w) => {
              const historyRaters = typeof LibraryModel !== "undefined" && LibraryModel.getRaters ? LibraryModel.getRaters(w) : null;
              const historyAverage = typeof LibraryModel !== "undefined" && LibraryModel.getAverageRating ? LibraryModel.getAverageRating(w) : null;
              const hasAvg = Number.isFinite(historyAverage);
              const avg = hasAvg ? formatAvg(historyAverage) : "";
              const historyRaterValue = (id, legacyValue) => {
                const state = historyRaters?.[id];
                if (state?.watched === false) return "No la ha visto";
                if (state && (state.rating === "" || state.rating == null)) return "Sin valorar";
                return state?.rating ?? legacyValue ?? "";
              };

              const na = (historyRaters?.adri?.watched === false ? "" : (w.notesAdri || "")).trim();
              const nl = (historyRaters?.laura?.watched === false ? "" : (w.notesLaura || "")).trim();
              ...
              return `
                <div class="historyItem">
                  ...
                  <div class="historyMeta">
                    ...
                    <span>${escapeHtml(getRaterDisplayName("adri"))}: ${escapeHtml(historyRaterValue("adri", w.ratingAdri))}</span>
                    <span>${escapeHtml(getRaterDisplayName("laura"))}: ${escapeHtml(historyRaterValue("laura", w.ratingLaura))}</span>
                  </div>
    ...
    const seenUnseenAdri = document.querySelector('#seenForm [data-rater-unseen="adri"]');
    const seenUnseenLaura = document.querySelector('#seenForm [data-rater-unseen="laura"]');
    const msgSeen = document.getElementById("msg-seen");
    function seenRaterState(id) {
      const adri=id==="adri", unseen=adri?seenUnseenAdri:seenUnseenLaura, field=adri?seenRatingAdri:seenRatingLaura, notes=adri?seenNotesAdri:seenNotesLaura;
      const watched=!Boolean(unseen?.checked), raw=(field?.value||"").toString().trim(), parsed=raw===""?"":Number(raw);
      return {watched, rating:watched&&Number.isFinite(parsed)?parsed:"", notes:(notes?.value||"").trim()};
    }
    function syncSeenRaterUI(id) {
      const adri=id==="adri", unseen=adri?seenUnseenAdri:seenUnseenLaura, field=adri?seenRatingAdri:seenRatingLaura, notes=adri?seenNotesAdri:seenNotesLaura;
      const widget=document.querySelector(`#seenForm [data-target="${field?.id||""}"]`), disabled=Boolean(unseen?.checked);
      if(notes) notes.disabled=disabled;
      if(widget){widget.classList.toggle("raterDisabled",disabled);widget.setAttribute("aria-disabled",disabled?"true":"false");}
      if(disabled&&widget&&typeof widget._setRating==="function") widget._setRating(""); else if(disabled&&field) field.value="";
    }
    [seenUnseenAdri,seenUnseenLaura].forEach(input=>input?.addEventListener("change",()=>syncSeenRaterUI(input.dataset.raterUnseen)));
    ...
    function openSeenModal() {
      ...
      syncSeenRaterUI("adri"); syncSeenRaterUI("laura");
  
      seenOverlay.classList.add("open");
      seenOverlay.setAttribute("aria-hidden", "false");

      // UX2.1 · Cada nueva valoración empieza desde la parte superior del modal.
      // El contenedor desplazable es .modalBody; no alteramos el scroll de la pantalla de fondo.
      const seenBody = seenOverlay.querySelector(".modalBody");
      if (seenBody) {
        seenBody.scrollTop = 0;
        requestAnimationFrame(() => { seenBody.scrollTop = 0; });
      }
    ...
    function closeSeenModal() {
      ...
      if (seenUnseenAdri) seenUnseenAdri.checked = false;
      if (seenUnseenLaura) seenUnseenLaura.checked = false;
      if (seenNotesAdri) {
        seenNotesAdri.disabled = false;
        seenNotesAdri.value = "";
      }
      if (seenNotesLaura) {
        seenNotesLaura.disabled = false;
        seenNotesLaura.value = "";
      }
      syncSeenRaterUI("adri");
      syncSeenRaterUI("laura");
    ...
    function saveSeenFromDetail() {
      ...
      const adriState = seenRaterState("adri");
      const lauraState = seenRaterState("laura");
      ...
      const ratingAdri=adriState.rating, ratingLaura=lauraState.rating;
      ...
      const existingSeen = LibraryRepository.findByTmdbId(toKey, item.tmdbId, item.id);
      if (existingSeen) {
        setMsg(
          msgSeen,
          kind === "series"
            ? "Esta serie ya tiene otro registro en Series vistas. Revisa el duplicado antes de marcarla como vista."
            : "Esta película ya tiene otro registro en Películas vistas. Revisa el duplicado antes de marcarla como vista.",
          "error"
        );
        return;
      }
      ...
      const entry = makeWatchEntry({
        ratingAdri,
        ratingLaura,
        notesAdri: adriState.notes,
        notesLaura: lauraState.notes,
        raters,
        at: Date.now()
      });
      ...
      // UX2.1 · Navegación dirigida al registro recién movido a Vistas.
      // Reutiliza UX1.1 para calcular la página, limpiar filtros/búsqueda si impiden
      // localizarlo, hacer scroll hasta la tarjeta y resaltarla brevemente.
      const targetScreen = kind === "series" ? "series-vistas" : "peliculas-vistas";
    ...
    function moveBackToPendientes() {
      ...
      const existingPending = LibraryRepository.findByTmdbId(toKey, item.tmdbId, item.id);
      if (existingPending) {
        closeConfirm();
        showToast(
          kind === "series"
            ? "Ya existe otro registro en Series pendientes"
            : "Ya existe otro registro en Películas pendientes"
        );
        return;
      }
      ...
      // FIX1.1 · Navegación dirigida al registro recién devuelto a Pendientes.
      // Calcula su página real con el orden/filtros actuales y, si estos impiden
      // localizarlo, reutiliza la lógica común para limpiar solo lo necesario,
      // posicionar la página correcta y resaltar el título.
      const targetScreen = kind === "series" ? "series-pendientes" : "peliculas-pendientes";
      setTimeout(() => {
        if (typeof LibraryTargetNavigation !== "undefined") {
          LibraryTargetNavigation.open({
            key: toKey,
