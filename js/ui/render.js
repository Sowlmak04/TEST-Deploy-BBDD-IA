// ---------- Render pendientes ----------
    ...
    function renderVistas(type){
      ...
      if (sorted.length === 0) {
        ...
        if (emptyEl){
          emptyEl.style.display = "block";
          emptyEl.innerHTML = (typeof window.getLibraryTemporalEmptyMessage === "function" && window.getLibraryTemporalEmptyMessage(screenName)) || `No hay resultados con los filtros actuales.`;
        }
        if(pagerEl){ pagerEl.style.display = "none"; pagerEl.innerHTML = ""; }
        return;
      }
      ...
      listEl.innerHTML = slice.map(raw => {
        ...
        meta.push(`<span>${escapeHtml(raterLabel("adri", getRaterDisplayName("adri")))}</span>`);
        meta.push(`<span>${escapeHtml(raterLabel("laura", getRaterDisplayName("laura")))}</span>`);
        if(item.year) meta.push(`<span>📅 ${escapeHtml(item.year)}</span>`);
        if(item.plannedDate) meta.push(`<span>🗓️ ${escapeHtml(item.plannedDate)}</span>`);
        const temporalMeta = typeof window.getLibraryTemporalFilterMeta === "function" ? window.getLibraryTemporalFilterMeta(raw, screenName) : null;
        if (temporalMeta) meta.push(`<span class="temporalFilterMeta">${escapeHtml(temporalMeta)}</span>`);
        if(isSeries && (item.currentSeason || item.currentEpisode)) {
          meta.push(`<span>▶️ ${escapeHtml([
            item.currentSeason ? `T${item.currentSeason}` : "",
            item.currentEpisode ? `E${item.currentEpisode}` : ""
