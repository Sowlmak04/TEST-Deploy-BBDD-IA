...
const PersonInsightsUI = (() => {
  ...
  function titleCard(item) {
    ...
    return `<button class="personInsightTitleCard personInsightTitleCardInteractive" type="button"
      data-person-title-id="${escape(item.id)}"
      data-person-title-kind="${escape(item.kind || "movie")}"
      data-person-title-status="${escape(item.status || "pendientes")}">
      <div>
  ...
  function discoveryCard(item) {
    ...
    return `<button class="personInsightDiscoveryCard personInsightDiscoveryCardInteractive" type="button"
      data-discovery-id="${escape(item.id)}"
      data-discovery-media-type="${escape(item.mediaType)}"
      data-discovery-title="${escape(item.title)}">
      ${item.posterUrl
  ...
  function open(identity, options = {}) {
  ...
  function close() {
    ...
    if (typeof KnowledgeNavigation !== "undefined" && KnowledgeNavigation.canBack()) {
      const previous = KnowledgeNavigation.back();
      if (typeof DetailView !== "undefined" && DetailView.restoreKnowledgeFrame(previous)) return;
    }
  }
  ...
  content?.addEventListener("click", event => {
    const discoveryButton = event.target.closest("[data-discovery-id]");
    if (discoveryButton) {
      if (typeof ExternalTitlePreview === "undefined" || !currentIdentity) return;
      ExternalTitlePreview.open({
        id: Number(discoveryButton.dataset.discoveryId),
        mediaType: discoveryButton.dataset.discoveryMediaType === "tv" ? "tv" : "movie",
        title: discoveryButton.dataset.discoveryTitle || "",
        personIdentity: currentIdentity
      });
      return;
    }

    const titleButton = event.target.closest("[data-person-title-id]");
    if (titleButton) {
      const detailContext = typeof DetailView !== "undefined" ? DetailView.contextSnapshot() : null;
      if (!detailContext) return;

      const targetContext = {
        id: titleButton.dataset.personTitleId,
        kind: titleButton.dataset.personTitleKind === "series" ? "series" : "movie",
        status: titleButton.dataset.personTitleStatus === "vistas" ? "vistas" : "pendientes",
        returnScreen: detailContext.returnScreen || "resumen-home"
      };
      ...
      if (typeof KnowledgeNavigation !== "undefined") KnowledgeNavigation.pushDetail(targetContext);
      DetailView.open(targetContext, { fromKnowledge: true });
      return;
    }
