// ---------- KB1.2.1 · Depuración de Person Insights ----------
const PersonInsightsUI = (() => {
  ...
  function appearanceLabels(titleItem) {
    ...
    appearances.forEach(appearance => {
      ...
      if (appearance?.type === "cast" && character) {
        labels.push(character);
      } else {
        labels.push(roleLabel(appearance));
      }
    });
  ...
  function titleCard(item) {
    ...
    return `<article class="personInsightTitleCard">
      <div>
        ...
        <span>${escape([year, kind, status].filter(Boolean).join(" · "))}</span>
      </div>
  ...
  function render() {
    ...
    content.innerHTML = `${roles.length || insight.identity.provisional ? `<div class="personInsightIdentityMeta">
