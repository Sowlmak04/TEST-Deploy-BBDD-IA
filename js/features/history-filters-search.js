// ---------- WatchLog (historial de visionados) ----------
    ...
    const filterGenresMeta = document.getElementById("filterGenresMeta");
    ...
    function setGenresExpanded(expanded){
      if (!filterGenres || !filterGenresToggle) return;
      filterGenres.hidden = !expanded;
      filterGenresToggle.setAttribute("aria-expanded", expanded ? "true" : "false");
      if (filterGenresMeta) {
        const selectedCount = readChecked(filterGenres).length;
        filterGenresMeta.textContent = selectedCount
          ? `${selectedCount} seleccionado${selectedCount === 1 ? "" : "s"} · ${expanded ? "Ocultar" : "Mostrar"}`
    ...
    function openFilterModal(scope){
      ...
      setGenresExpanded(pendingFilter.genres.length > 0);
    ...
    filterGenresToggle?.addEventListener("click", () => {
      setGenresExpanded(filterGenresToggle.getAttribute("aria-expanded") !== "true");
    });
    filterGenres?.addEventListener("change", () => {
      if (filterGenresToggle?.getAttribute("aria-expanded") === "true") {
        setGenresExpanded(true);
      }
    });

    filterClear.addEventListener("click", () => {
      [filterGenres, filterPriorities, filterTags]
        .filter(Boolean)
