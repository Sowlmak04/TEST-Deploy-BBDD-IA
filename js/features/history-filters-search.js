// ---------- WatchLog (historial de visionados) ----------
    ...
    const filterWatchedDateStatus = document.getElementById("filterWatchedDateStatus");
    ...
    function watchedDateControlLabel(mode){
      const now = new Date();
      const month = Number(filterWatchedMonth?.value) || now.getMonth() + 1;
      const year = Number(filterWatchedYear?.value) || now.getFullYear();
      if (mode === "current_month") return "Este mes";
    ...
    function syncWatchedDateControls(){
      ...
      // HISTORY1.1a: Mes y Año permanecen accesibles. Si el usuario los cambia,
      // el modo se sincroniza automáticamente para que el selector superior
      // refleje (y no invalide) la intención expresada abajo.
      filterWatchedDateSelectors.hidden = false;
      if (filterWatchedMonthLabel) filterWatchedMonthLabel.hidden = false;

      const active = mode !== "any";
      filterWatchedDateSection?.classList.toggle("isFilterActive", active);
      filterWatchedDateMode.classList.toggle("isActiveControl", active);
      filterWatchedMonthLabel?.classList.toggle("isActiveControl", mode === "month");
      filterWatchedYear?.closest(".filterSelectLabel")?.classList.toggle("isActiveControl", mode === "month" || mode === "year");

      if (filterWatchedDateStatus) {
        const label = watchedDateControlLabel(mode);
        filterWatchedDateStatus.textContent = label;
        filterWatchedDateStatus.hidden = !label;
      }
    }
    ...
    filterWatchedMonth?.addEventListener("change", () => {
      // Elegir un mes concreto tiene prioridad sobre "Cualquier fecha" o "Este mes".
      if (filterWatchedDateMode) filterWatchedDateMode.value = "month";
      syncWatchedDateControls();
    });
    filterWatchedYear?.addEventListener("change", () => {
      if (!filterWatchedDateMode) return;
      // Si ya estamos construyendo un mes concreto, cambiar el año conserva ese mes.
      // En cualquier otro modo, elegir un año expresa una consulta anual concreta.
      if (filterWatchedDateMode.value !== "month") filterWatchedDateMode.value = "year";
      syncWatchedDateControls();
    });
    ...
    filterClear.addEventListener("click", () => {
      ...
      if (filterWatchedMonth) filterWatchedMonth.value = String(new Date().getMonth() + 1);
      if (filterWatchedYear) filterWatchedYear.value = String(new Date().getFullYear());
      syncWatchedDateControls();
...
window.openLibraryWithPersonalAvailabilityFilter = function(scope) {
  ...
  // SUMMARY3.1a: si el acceso nace en Resumen > Biblioteca > Disponible ahora,
  // el Volver de la colección debe regresar al origen contextual.
  const backButton = document.querySelector(`.screen[data-screen="${scope}"] .backBtn[data-screen]`);
  if (backButton) {
    if (!backButton.dataset.availableReturnPrev) {
      backButton.dataset.availableReturnPrev = backButton.dataset.screen || (scope.startsWith("series-") ? "series-home" : "peliculas-home");
    }
    backButton.dataset.screen = "resumen-home";
    backButton.dataset.availableReturn = "true";
  }
...
// SUMMARY3.1a: el destino contextual de Volver solo se consume una vez.
// El listener directo de navegación lee primero data-screen="resumen-home"; al
// burbujear el click restauramos el destino normal para futuras entradas.
document.addEventListener("click", event => {
  const backButton = event.target.closest('.backBtn[data-available-return="true"]');
  if (!backButton) return;
  const previous = backButton.dataset.availableReturnPrev;
  if (previous) backButton.dataset.screen = previous;
  delete backButton.dataset.availableReturn;
  delete backButton.dataset.availableReturnPrev;
});

// HISTORY1.3: puente contextual entre Explorar actividad y las bibliotecas vistas.
let historyLibraryReturnContext = null;

function historyPeriodLabel(context) {
  if (!context) return "";
  if (context.mode === "year") return String(context.year || "");
  const date = new Date(Number(context.year), Math.max(0, Number(context.month) - 1), 1);
  return new Intl.DateTimeFormat("es-ES", { month: "long", year: "numeric" }).format(date);
}

function syncHistoryLibraryReturnBanner() {
  document.querySelectorAll("[data-history-library-return]").forEach(container => {
    const active = Boolean(historyLibraryReturnContext && container.dataset.historyLibraryReturn === historyLibraryReturnContext.scope);
    container.hidden = !active;
    const label = container.querySelector(".historyLibraryReturnLabel");
    if (label) label.textContent = active ? `Periodo: ${historyPeriodLabel(historyLibraryReturnContext)}` : "";
  });
}

function clearHistoryLibraryReturnContext() {
  historyLibraryReturnContext = null;
  syncHistoryLibraryReturnBanner();
}

window.openLibraryFromStatisticsHistory = function({ kind, mode, year, month, returnSnapshot } = {}) {
  const scope = kind === "series" ? "series-vistas" : "peliculas-vistas";
  const normalizedMode = mode === "year" ? "year" : "month";
  ...
  setFilter(scope, {
    genres: [], priorities: [], tags: [], platforms: [],
    myPlatformsOnly: false, personalAvailabilityOnly: false, ownedPhysicalOnly: false, favoritesOnly: false,
    watchedDateMode: normalizedMode,
    watchedMonth: normalizedMode === "month" ? normalizedMonth : null,
    watchedYear: normalizedYear
  });
  setSearch(scope, "");
  setPageState(scope, 1);
  updateFilterBadge(scope);
  ...
  const box = document.querySelector(`.searchBox[data-search-box="${scope}"]`);
  if (box) box.classList.remove("open");
  document.querySelector(`.screen[data-screen="${scope}"]`)?.classList.remove("searchOpen");

  historyLibraryReturnContext = {
    scope,
    mode: normalizedMode,
    year: normalizedYear,
    month: normalizedMonth,
    returnSnapshot: returnSnapshot || null
  };
  syncHistoryLibraryReturnBanner();
...
document.addEventListener("click", event => {
  const returnButton = event.target.closest(".historyLibraryReturn");
  if (returnButton && historyLibraryReturnContext) {
    const snapshot = historyLibraryReturnContext.returnSnapshot;
    clearHistoryLibraryReturnContext();
    if (snapshot && typeof window.restoreStatisticsHistoryReturn === "function") {
      window.restoreStatisticsHistoryReturn(snapshot);
    } else {
      setMainTab("resumen");
      showScreen("resumen-home");
    }
    return;
  }

  if (!historyLibraryReturnContext) return;
  const normalNavigation = event.target.closest("button[data-screen], .tab[data-main]");
  if (normalNavigation) clearHistoryLibraryReturnContext();
});
