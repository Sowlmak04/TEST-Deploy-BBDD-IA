...
function priorityTimingLabel(plannedDays, ageDays) {
  if (Number.isFinite(plannedDays)) {
    if (plannedDays === 0) return "Prevista para hoy";
    if (plannedDays === 1) return "Prevista para mañana";
    if (plannedDays > 1) return `Prevista para dentro de ${plannedDays} días`;
    if (plannedDays === -1) return "Prevista para ayer";
    return `Prevista hace ${Math.abs(plannedDays)} días`;
  }

  return ageDays ? `${ageDays} d pendiente` : "Recién añadido";
}
  ...
  return `
    <section class="statsSection productivitySection" aria-labelledby="priority-title">
      ...
      ${
        items.length
          ? `<div class="productivityList">
              ${items.map(({ item, ageDays, plannedDays }) => `
                ${productivityButton(item, `
                  ${trackingPoster(item)}
                  <span class="productivityMain">
                    <strong>${escapeHtml(item.title || "Sin título")}</strong>
                  ...
                  <span class="productivityCallout ${Number.isFinite(plannedDays) && plannedDays < 0 ? "isOverdue" : ""}">
                    ${escapeHtml(priorityTimingLabel(plannedDays, ageDays))}
                  </span>
                `)}
              `).join("")}
            </div>`
  ...
  return `
    <section class="statsSection" aria-labelledby="stats-affinity-title">
      ...
      <div class="statsAffinityColumns">
        ${renderAffinityList("Mayor coincidencia", affinity.coincidences, "No hay valoraciones comparables.", "stats-coincidences-title")}
