...
function renderTemporalHistoryResults(result) {
  ...
  return `
    ...
    <div class="historyLibraryActions" aria-label="Abrir bibliotecas filtradas por este periodo">
      ${result.movies > 0 ? `<button type="button" class="historyLibraryAction" data-history-library="peliculas">Ver películas <span>${escapeHtml(result.movies)}</span></button>` : ""}
      ${result.series > 0 ? `<button type="button" class="historyLibraryAction" data-history-library="series">Ver series <span>${escapeHtml(result.series)}</span></button>` : ""}
    </div>
    ${result.mode === "year"
