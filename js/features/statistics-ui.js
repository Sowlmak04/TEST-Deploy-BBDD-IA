...
let temporalHistoryYearExpanded = false;
...
function renderRecentActivitySummary(activity) {
  ...
  return `
    <section class="statsSection" aria-labelledby="stats-recent-summary-title">
      <div class="statsSectionHead">
        <div>
          <span class="statsEyebrow">Últimos ${escapeHtml(activity?.months || 6)} meses</span>
          <h2 id="stats-recent-summary-title">Resumen del periodo</h2>
        </div>
      </div>
      <div class="statsActivityMetricGrid">
        ${statCard(total, "Títulos vistos", `${movies} películas · ${series} series`)}
        ${statCard(
          formatActivityAverage(activity?.averagePerMonth),
          "Media mensual",
          "Títulos registrados como vistos por mes"
        )}
        ${statCard(
...
function renderTemporalHistoryResults(result) {
  ...
  return `
    ...
    ${result.mode === "year"
      ...
      : `
        <div class="historyTitleList">
          ${result.rows.map(row => {
            const item = row.item;
            const repeat = row.eventCount > 1 ? ` · ${row.eventCount} visionados` : "";
            return productivityButton(item, `
              ${trackingPoster(item)}
              <span class="productivityMain">
                <strong>${escapeHtml(item.title || "Sin título")}</strong>
                <small>${escapeHtml(`${item.kind === "series" ? "Serie" : "Película"} · ${formatHistoryDate(row.latestAt)}${repeat}`)}</small>
              </span>
              <span class="historyRating">${escapeHtml(formatRating(row.rating))}</span>
            `);
          }).join("")}
        </div>
      `
...
function renderTemporalHistoryYearList(result) {
  const rows = temporalHistoryYearExpanded ? result.rows : result.rows.slice(0, 10);
  ...
  return `
    ...
    ${hasMore ? `
      <button class="historyYearToggle" type="button" data-history-year-toggle>
        ${temporalHistoryYearExpanded ? "Ver menos" : `Ver todos (${result.rows.length})`}
      </button>
    ` : ""}
  `;
}
  ...
  return `
    <section class="statsSection" aria-labelledby="stats-ratings-title">
      ...
      <div class="statsActivityMetricGrid">
        ...
        ${statCard(formatRating(ratings.adriAverage), `Media ${getRaterDisplayName("adri")}`, "Solo valoraciones registradas")}
        ${statCard(formatRating(ratings.lauraAverage), `Media ${getRaterDisplayName("laura")}`, "Solo valoraciones registradas")}
      </div>
    </section>
  `;
}
...
function renderRatingDistribution(ratings) {
  ...
  return `
    <section class="statsSection" aria-labelledby="stats-rating-distribution-title">
      <div class="statsSectionHead">
        <div>
          <span class="statsEyebrow">Cómo utilizáis la escala</span>
...
document.addEventListener("click", event => {
  ...
  if (historyMode) {
    ...
    temporalHistoryYearExpanded = false;
    const panel = document.querySelector("[data-stats-tab-panel]");
    if (panel) panel.innerHTML = renderStatisticsTab("activity", StatisticsService.buildDashboard());
    return;
  }

  const yearToggle = event.target.closest("[data-history-year-toggle]");
  if (yearToggle) {
    temporalHistoryYearExpanded = !temporalHistoryYearExpanded;
