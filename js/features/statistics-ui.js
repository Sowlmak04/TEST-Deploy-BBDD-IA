...
function renderActivityHistory(dashboard) {
  ...
  return `
    <section class="statsSection" aria-labelledby="stats-history-title">
      <div class="statsSectionHead">
        <div>
      ...
      <div class="statsActivityMetricGrid">
        ${statCard(
          Number(coverage.withDate || 0),
          "Con fecha",
          `${Number(coverage.percentage || 0)}% de los títulos vistos`
        )}
        ${statCard(
          Number(coverage.withoutDate || 0),
          "Sin fecha",
          "No se incluyen en las estadísticas temporales"
        )}
        ${statCard(
      ...
      <p class="statsSectionDetail">
        Estas cifras describen títulos con fecha de visionado registrada; no reconstruyen sesiones ni episodios históricos.
      </p>
    </section>
  `;
}
