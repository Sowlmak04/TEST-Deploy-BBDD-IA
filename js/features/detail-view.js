// ---------- KB1.3 · Navegación cruzada de conocimiento ----------
const KnowledgeNavigation = (() => {
  ...
  function resetWithDetail(context) {
    frames = [{ type: "detail", context: cloneContext(context) }];
  }

  function pushPerson(identity) {
    const key = String(identity || "").trim();
    if (!key) return;
    frames.push({ type: "person", identity: key });
  }

  function pushDetail(context) {
    frames.push({ type: "detail", context: cloneContext(context) });
  }
  ...
  return Object.freeze({ resetWithDetail, pushPerson, pushDetail, current, canBack, back, clear });
})();
...
const DetailView = (() => {
  ...
  function personIdentityKey(person) {
    return typeof PersonInsightsService !== "undefined"
      ? PersonInsightsService.identityOf(person)?.key || ""
      : "";
  }

  function personButton(person, extraClass = "") {
    const name = String(person?.name || "").trim();
    const identityKey = personIdentityKey(person);
    if (!name) return "";
    if (!identityKey) return `<span>${escape(name)}</span>`;
    return `<button class="detailPersonLink ${escape(extraClass)}" type="button" data-person-identity="${escape(identityKey)}">${escape(name)}</button>`;
  }
  ...
  function castMarkup(item) {
    ...
    return `<div class="detailCastGrid">${cast.map(person => {
      const identityKey = personIdentityKey(person);
      const tag = identityKey ? "button" : "article";
      const attrs = identityKey ? ` type="button" data-person-identity="${escape(identityKey)}"` : "";
      return `<${tag} class="detailCastCard${identityKey ? " detailCastCardInteractive" : ""}"${attrs}>
  ...
  function crewMarkup(item) {
    const groups = [];
    const creators = list(item.creators).filter(person => String(person?.name || "").trim());
    ...
    list(item.crew).forEach(person => {
      ...
      grouped.get(label).push(person);
    });

    grouped.forEach((people, label) => groups.push([label, people]));
    groups.push(["Música / Compositor", composers]);

    return `<dl class="detailCrewList">${groups.map(([label, people]) => {
      const unique = [];
      const seen = new Set();
      people.forEach(person => {
        const key = personIdentityKey(person) || `name:${String(person?.name || "").trim().toLocaleLowerCase("es-ES")}`;
        if (seen.has(key)) return;
        seen.add(key);
        unique.push(person);
      });
      return `<div><dt>${escape(label)}</dt><dd class="${unique.length ? "" : "detailEmpty"}">${unique.length ? unique.map(person => personButton(person)).join('<span class="detailPersonSeparator">, </span>') : "No disponible"}</dd></div>`;
    }).join("")}</dl>`;
  }
  ...
  function availabilityMarkup(item) {
    ...
    const ownedPhysical = item.ownedPhysical
      ? `<div class="detailOwnedPhysical" role="status">
          <span class="detailOwnedPhysicalIcon" aria-hidden="true">▣</span>
          <div><strong>Colección propia</strong><span>Tienes una copia física de este título.</span></div>
        </div>`
      : "";
  ...
  function render() {
    ...
    const summary = `<div class="detailSynopsis ${synopsis ? "" : "detailEmpty"}">${escape(synopsis || "Sin sinopsis disponible.")}</div>
      <dl class="detailMetaGrid">
        ...
        ${row("Estado de producción", productionStatusLabel(item.productionStatus || item.tmdbStatus))}
  ...
  function open(value, options = {}) {
    const fromKnowledge = options?.fromKnowledge === true;
    context = { ...value, returnScreen: value?.returnScreen || "resumen-home" };

    if (!fromKnowledge) {
      KnowledgeNavigation.resetWithDetail(context);
  ...
  function restoreKnowledgeFrame(frame) {
    if (!frame) return false;
    if (frame.type === "detail" && frame.context) {
      open(frame.context, { fromKnowledge: true });
      return true;
    }
    if (frame.type === "person" && frame.identity && typeof PersonInsightsUI !== "undefined") {
      PersonInsightsUI.open(frame.identity, { fromKnowledge: true });
      return true;
    }
    return false;
  }
  ...
  function close() {
    if (KnowledgeNavigation.canBack()) {
      const previous = KnowledgeNavigation.back();
      if (restoreKnowledgeFrame(previous)) return;
    }

    KnowledgeNavigation.clear();
    document.body.classList.remove("person-insights-open");
    document.body.classList.remove("detail-view-open");

    if (context?.historyReturn && typeof window.restoreStatisticsHistoryReturn === "function") {
      if (window.restoreStatisticsHistoryReturn(context.historyReturn)) return;
    }
  ...
  content?.addEventListener("click", event => {
    ...
    const personButton = event.target.closest("[data-person-identity]");
    if (personButton) {
      if (typeof PersonInsightsUI !== "undefined") {
        KnowledgeNavigation.pushPerson(personButton.dataset.personIdentity);
        PersonInsightsUI.open(personButton.dataset.personIdentity, { fromKnowledge: true });
      }
      return;
    }
  ...
  return { open, render, contextSnapshot, restoreKnowledgeFrame };
})();
