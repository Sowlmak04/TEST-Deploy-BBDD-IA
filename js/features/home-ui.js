...
function homeProgressPercentage(item) {
  if (typeof SeriesProgressService === "undefined" ||
      typeof SeriesProgressService.snapshot !== "function") {
    return null;
  }

  const percentage = SeriesProgressService.snapshot(item)?.percentage;
...
function renderHomeDashboard() {
  ...
  // FIX1.1 · Un título visible en "Continuar viendo" no compite a la vez en
  // "Qué ver ahora". La misma lista que se renderiza arriba define la exclusión,
  // y también se aplica a "Otra opción" porque homeRecommendations ya queda filtrado.
  homeRecommendations = Array.isArray(dashboard.availableRecommendations)
    ? dashboard.availableRecommendations.filter(entry => {
        const id = entry?.item?.id;
        return id == null || !continuingIds.has(String(id));
      })
    : [];
      ...
      <section class="homeSection">
        ...
        <div class="homeContinueList">
          ${continuing.map(({ item, season, episode, nextSeason, nextEpisode }) => {
            const percentage = homeProgressPercentage(item);
            ...
            return `
              ...
              >
                ...
                <span class="homeContinueBody">
                  ...
                  ${percentage !== null ? `
                    <span class="seriesProgressTrack homeContinueProgress" aria-hidden="true">
                      <span class="seriesProgressFill" style="width:${homeEscape(percentage)}%"></span>
                    </span>
                  ` : ""}
