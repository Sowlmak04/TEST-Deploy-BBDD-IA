// ---------- Rating widget (10 estrellas, medias con click) ----------
    ...
    function resetRaterParticipation(form) {
      form.querySelectorAll("[data-rater-unseen]").forEach(x => x.checked = false);
      ["adri","laura"].forEach(id => syncRaterFormUI(form,id));
    }
    ...
    function saveVista({ form, key, msgEl }) {
      ...
      if (!adriState.watched && !lauraState.watched) {
        setMsg(msgEl, "Al menos Adri o Laura debe haber visto el título.", "error");
        return;
      }
