...
const ExternalTitlePreview = (() => {
  ...
  function prepareAddedPendingTarget(screenName, itemId) {
    const keyByScreen = {
    ...
    let targetIndex = visibleItems.findIndex(item => String(item?.id || "") === String(itemId));
    ...
    if (targetIndex < 0) {
      ...
      targetIndex = visibleItems.findIndex(item => String(item?.id || "") === String(itemId));
    }
  ...
  function highlightAddedPending(itemId) {
    if (!itemId) return;
    window.setTimeout(() => {
      const target = document.querySelector(`.itemCard[data-id="${CSS.escape(String(itemId))}"]`);
      if (!target) return;
  ...
  document.addEventListener("library:pending-added", event => {
    ...
    prepareAddedPendingTarget(target.libraryScreen, addedItemId);
    if (typeof showScreen === "function") showScreen(target.libraryScreen);
    highlightAddedPending(addedItemId);
