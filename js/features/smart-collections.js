const SmartCollectionsUI = (() => {
  ...
  const definitions = [
    ...
    {
      id: "owned-physical",
      title: "Colección propia",
      description: "Películas y series que forman parte de tu colección propia.",
      icon: "▣",
      matches: item => Boolean(item.ownedPhysical)
    },
    {
      id: "available-for-me",
      title: "Disponibles para ti",
      description: "Títulos pendientes disponibles en tus plataformas o en tu colección propia.",
      icon: "✓",
      matches: item =>
    ...
    },
    {
      ...
      matches: item => {
        ...
        return Number.isFinite(average) && average >= 9;
      }
    },
    {
      id: "tagged",
  ...
  function buildBadges(item) {
    ...
    if (item.ownedPhysical) {
      badges.push(
        '<span class="personalBadge ownedPhysicalBadge">▣ Colección propia</span>'
      );
    }
  ...
  function openCollection(collectionId, type = "all") {
    const definition = definitions.find(item => item.id === collectionId);
    if (!definition) return false;

    state.type = ["all", "series", "peliculas"].includes(type) ? type : "all";
    state.collection = collectionId;
    state.page = 1;
    render();

    requestAnimationFrame(() => {
      const screen = document.querySelector('.screen[data-screen="colecciones-inteligentes"]');
      const results = document.getElementById("smart-collection-results");
      if (!screen || !results) return;
      screen.scrollTo({ top: Math.max(0, results.offsetTop - 10), behavior: "auto" });
    });
