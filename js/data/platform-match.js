(function () {
  ...
  function matchesProvider(item, provider) {
    const available = Array.isArray(item?.watchProviders)
      ? item.watchProviders
      : [];
    if (!available.length || !provider) return false;

    const id = providerId(provider);
    if (id && available.some(candidate => providerId(candidate) === id)) return true;

    const name = normalize(providerName(provider));
    return Boolean(name && available.some(candidate =>
      normalize(providerName(candidate)) === name
    ));
  }
