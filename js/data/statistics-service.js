...
const StatisticsService = (() => {
  ...
  function activityWindow(items, months = 6) {
    const safeMonths = [1, 3, 6, 12].includes(Number(months)) ? Number(months) : 6;
    const now = new Date();
    const buckets = [];
    ...
    for (let offset = safeMonths - 1; offset >= 0; offset -= 1) {
      ...
      buckets.push({
        ...
        fullLabel: new Intl.DateTimeFormat("es-ES", { month: "long", year: "numeric" }).format(date),
        count: 0,
        movies: 0,
        series: 0
      });
    ...
    items.forEach(item => {
      watchEvents(item).forEach(event => {
    ...
    const mostActive = buckets.reduce((best, bucket) => !best || bucket.count > best.count ? bucket : best, null);
    ...
    return {
      ...
      movies,
      series,
      averagePerMonth: safeMonths > 0 ? total / safeMonths : 0,
      mostActive: mostActive && mostActive.count > 0
        ? { key: mostActive.key, label: mostActive.fullLabel, count: mostActive.count }
        : null
    };
  }
  ...
  function watchEvents(item) {
    const log = Array.isArray(item?.watchLog) ? item.watchLog : [];
    const events = log
      .map(entry => {
        const at = Number(entry?.at);
        if (!Number.isFinite(at) || at <= 0) return null;
        const eventRating = numericRating(
          entry?.rating ?? entry?.averageRating ?? entry?.jointRating
        );
        return { at, rating: eventRating };
      })
      .filter(Boolean);

    if (events.length) return events.sort((a, b) => a.at - b.at);

    const fallback = Number(item?.watchedAt);
    return Number.isFinite(fallback) && fallback > 0
      ? [{ at: fallback, rating: null }]
      : [];
  }

  function temporalHistoryQuery({ mode = "month", year, month } = {}) {
    const data = allData();
    const watched = [...data.seriesVistas, ...data.peliculasVistas];
    const now = new Date();
    const selectedYear = Number.isInteger(Number(year)) ? Number(year) : now.getFullYear();
    const selectedMonth = Number.isInteger(Number(month)) ? Number(month) : now.getMonth() + 1;
    ...
    const rows = [];
    watched.forEach(item => {
      const matches = watchEvents(item).filter(event => {
        const date = new Date(event.at);
      ...
      const eventRatings = matches.map(event => event.rating).filter(Number.isFinite);
      rows.push({
        item,
        events: matches,
        eventCount: matches.length,
        latestAt: latest.at,
        rating: eventRatings.length ? eventRatings[eventRatings.length - 1] : ratingPair(item)
      });
    });

    rows.sort((a, b) => b.latestAt - a.latestAt || String(a.item?.title || "").localeCompare(String(b.item?.title || ""), "es"));

    const movies = rows.filter(row => row.item?.kind !== "series").length;
    const series = rows.filter(row => row.item?.kind === "series").length;
    const eventCount = rows.reduce((sum, row) => sum + row.eventCount, 0);
    const ratings = rows.map(row => row.rating).filter(Number.isFinite);

    const genres = new Map();
    rows.forEach(row => {
      splitValues(row.item?.genre).forEach(label => {
        const key = normalizeLabel(label);
        if (!key) return;
        if (!genres.has(key)) genres.set(key, { label, count: 0 });
        genres.get(key).count += 1;
      });
    });
    const topGenre = [...genres.values()]
      .sort((a, b) => b.count - a.count || a.label.localeCompare(b.label, "es"))[0] || null;
  ...
  function temporalHistoryOptions() {
    const data = allData();
    const watched = [...data.seriesVistas, ...data.peliculasVistas];
    const years = new Set();
    watched.forEach(item => {
      watchEvents(item).forEach(event => years.add(new Date(event.at).getFullYear()));
    });
    years.add(new Date().getFullYear());
    return [...years].filter(Number.isFinite).sort((a, b) => b - a);
  }

  function buildDashboard({ activityMonths = 6 } = {}) {
    const data = allData();
    const watched = [...data.seriesVistas, ...data.peliculasVistas];
    ...
    const recentActivity = activityWindow(watched, activityMonths);
