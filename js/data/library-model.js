...
const LibraryModel = (() => {
  ...
  const DEFAULT_RATER_IDS = Object.freeze(["adri", "laura"]);

  function legacyRaterFields(id) {
    const suffix = id === "adri" ? "Adri" : id === "laura" ? "Laura" : "";
    return suffix
      ? { rating: `rating${suffix}`, notes: `notes${suffix}` }
      : { rating: "", notes: "" };
  }

  function normalizeRaterState(value, legacy = {}) {
    const source = value && typeof value === "object" && !Array.isArray(value)
      ? value
      : {};
    const rating = normalizeRating(
      source.rating !== undefined ? source.rating : legacy.rating
    );
    const notes = normalizeText(
      source.notes !== undefined ? source.notes : legacy.notes
    );
    const watched = typeof source.watched === "boolean"
      ? source.watched
      : rating !== "";

    return { watched, rating, notes };
  }

  function normalizeRaters(source) {
    const base = source && typeof source === "object" && !Array.isArray(source)
      ? source
      : {};
    const rawRaters = base.raters && typeof base.raters === "object" && !Array.isArray(base.raters)
      ? base.raters
      : {};
    const ids = new Set([...DEFAULT_RATER_IDS, ...Object.keys(rawRaters)]);
    const raters = {};

    ids.forEach(id => {
      const fields = legacyRaterFields(id);
      raters[id] = normalizeRaterState(rawRaters[id], {
        rating: fields.rating ? base[fields.rating] : undefined,
        notes: fields.notes ? base[fields.notes] : undefined
      });
    });
  ...
  function getRaters(source) {
    return normalizeRaters(source);
  }

  function getRaterState(source, raterId) {
    const id = normalizeText(raterId).toLocaleLowerCase("es-ES");
    if (!id) return normalizeRaterState(null);
    return normalizeRaters(source)[id] || normalizeRaterState(null);
  }


  function numericRaterRating(source, raterId) {
    const state = getRaterState(source, raterId);
    if (!state.watched || state.rating === "" || state.rating == null) return null;
    const number = Number(state.rating);
    return Number.isFinite(number) ? number : null;
  }

  function getRealRatings(source) {
    return Object.entries(getRaters(source))
      .map(([id, state]) => ({ id, watched: state.watched, rating: numericRaterRating(source, id), notes: state.notes }))
      .filter(entry => Number.isFinite(entry.rating));
  }

  function getAverageRating(source) {
    const values = getRealRatings(source).map(entry => entry.rating);
    return values.length ? values.reduce((sum, value) => sum + value, 0) / values.length : null;
  }

  function hasComparableRatings(source, raterIds = DEFAULT_RATER_IDS) {
    return raterIds.every(id => Number.isFinite(numericRaterRating(source, id)));
  }

  function syncLegacyRaterFields(target) {
    const next = { ...target, raters: normalizeRaters(target) };
    DEFAULT_RATER_IDS.forEach(id => {
      const fields = legacyRaterFields(id);
      const state = next.raters[id];
      next[fields.rating] = state.watched ? state.rating : "";
      next[fields.notes] = state.watched ? state.notes : "";
    });
    return next;
  }

  function setRaterState(source, raterId, patch = {}) {
    const id = normalizeText(raterId).toLocaleLowerCase("es-ES");
    if (!id) return syncLegacyRaterFields(source || {});
    const base = source && typeof source === "object" ? source : {};
    const raters = normalizeRaters(base);
    raters[id] = normalizeRaterState({ ...raters[id], ...patch });
    return syncLegacyRaterFields({ ...base, raters });
  }

  function normalizeSeasonRatings(value) {
    const source = value && typeof value === "object" && !Array.isArray(value)
      ? value
      : {};
    const normalized = {};

    Object.entries(source).forEach(([seasonKey, ratings]) => {
      const seasonNumber = Number(seasonKey);
      if (!Number.isInteger(seasonNumber) || seasonNumber <= 0) return;

      const adri = normalizeRating(ratings?.adri);
      const laura = normalizeRating(ratings?.laura);
      if (adri === "" && laura === "") return;

      normalized[String(seasonNumber)] = { adri, laura };
    });
  ...
  function normalizeItem(item, context = {}, usedIds = null) {
    ...
    const normalized = {
      ...
      ownedPhysical: Boolean(source.ownedPhysical),
    ...
    normalized.raters = normalizeRaters(source);
    ...
    if (kind === "series") {
      ...
      normalized.seasonRatings = normalizeSeasonRatings(source.seasonRatings);
    ...
    if (status === "vistas") {
      ...
      normalized.raters = normalizeRaters(lastWatch);
    } else {
