...
const TMDbClient = (() => {
  ...
  async function details(id, kind) {
    ...
    return {
      ...
      backdropUrl: data.backdrop_path ? `${DEFAULT_IMAGE_BASE}w780${data.backdrop_path}` : "",
  ...
  async function personDetails(id) {
    ...
    if (!Number.isInteger(personId) || personId <= 0) {
      throw new Error("TMDB_INVALID_PERSON_ID");
    }

    return request(`/person/${personId}`);
  }

  async function personCombinedCredits(id) {
    const personId = Number(id);
    if (!Number.isInteger(personId) || personId <= 0) {
      throw new Error("TMDB_INVALID_PERSON_ID");
    }
