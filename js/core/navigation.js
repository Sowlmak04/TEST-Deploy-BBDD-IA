    ...
    function scrollPagedListToStart(screenName) {
      requestAnimationFrame(() => {
        const screen = document.querySelector(`.screen[data-screen="${screenName}"]`);
        const list = document.getElementById(`list-${screenName}`);
        if (!screen || !list) return;
        const targetTop = Math.max(0, list.offsetTop - 8);
    ...
    document.addEventListener("click", (e) => {
      ...
      const previousPage = pageState[screenName] || 1;
      ...
      scrollPagedListToStart(screenName);
    });
    ...
    function showScreen(screenName) {
      ...
      if (screenName.startsWith("anadir-") && activeScreen) {
        ...
        if (form && !form.dataset.editId) {
          ...
          if (typeof resetRaterParticipation === "function") resetRaterParticipation(form);
      ...
      if (
        screenName === "configurar-valoradores" &&
        typeof RatersConfigUI !== "undefined"
      ) {
        RatersConfigUI.open();
      }
    ...
    function setMainTab(main){
      ...
      if(main === "inicio")    showScreen("inicio-home");
