...
function downloadStorageDiagnostic() {
  ...
  setTimeout(() => URL.revokeObjectURL(url), 1000);
    ...
    function buildExportObject() {
      ...
      const raters = (typeof RatersConfigRepository !== "undefined")
        ? RatersConfigRepository.load()
        : { schemaVersion: 1, raters: [{ id: "adri", name: "Adri" }, { id: "laura", name: "Laura" }], savedAt: null };
    ...
    function readImportedRaters(parsedWrap) {
      const raw = parsedWrap?.raw || parsedWrap || {};
      const data = parsedWrap?.data || raw?.data || raw;
      const candidates = [raw?.settings?.raters, data?.settings?.raters, data?.raters, raw?.raters];
      const source = candidates.find(value => value !== undefined);
      if (source === undefined) return { present: false, valid: false, config: null };
      if (typeof RatersConfigRepository === "undefined") return { present: true, valid: false, config: null };
      try { return { present: true, valid: true, config: RatersConfigRepository.normalizeConfig(source, { strict: true }) }; }
      catch { return { present: true, valid: false, config: null }; }
    }

    function restoreImportedRaters(parsedWrap) {
      const imported = readImportedRaters(parsedWrap);
      if (!imported.present) return { restored: false, reason: "absent" };
      if (!imported.valid) { console.warn("[Importación] Bloque raters inválido; se conserva la configuración actual."); return { restored: false, reason: "invalid" }; }
      RatersConfigRepository.save(imported.config);
    ...
    function prepareImportReplace(parsedWrap) {
      ...
      return {
        ...
        raters: readImportedRaters(parsedWrap)
      };
    }
    ...
    function importReplaceAll(parsedWrap) {
      ...
      const ratersRestore = restoreImportedRaters(parsedWrap);
      ...
      if (ratersRestore?.restored) {
        try {
          if (typeof RatersConfigUI !== "undefined" && typeof RatersConfigUI.open === "function") RatersConfigUI.open();
        } catch (error) { console.warn("[Importación] No se pudo refrescar Configurar valoradores.", error); }
      }
