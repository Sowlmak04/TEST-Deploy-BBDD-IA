...
if (importFileMerge) {
  ...
  importFileMerge.addEventListener("change", async () => {
    ...
    try {
      ...
      const preview = getImportMergePreview(parsed);
      const confirmed = window.confirm(
        "Vas a MEZCLAR este backup con tu biblioteca actual.\n\n" +
        `Biblioteca actual: ${preview.currentCount} registros.\n` +
        `Archivo importado: ${preview.importedCount} registros.\n\n` +
        "Los registros actuales no se eliminarán por simples coincidencias de título. " +
        "Las coincidencias identificadas con seguridad podrán actualizarse con los datos importados.\n\n" +
        "¿Quieres continuar?"
      );
...
window.openSmartCollectionFromSummary = function(collectionId) {
  smartCollectionsReturnScreen = "resumen-home";
  showScreen("colecciones-inteligentes");
  if (typeof SmartCollectionsUI !== "undefined") {
    SmartCollectionsUI.openCollection(collectionId, "all");
  }
};
